...
DEFAULT_API_SCHEMA_NAME=magi_api
DEFAULT_SCHEMA_NAME=magi_app
DEFAULT_APP_CONTEXT_NAME=magi_app
...
if [[ "$APP_CONTEXT_NAME" != "$DEFAULT_APP_CONTEXT_NAME" ]]; then
    ...
    if [[ $IS_DOCKER = true ]]; then
        ${SED_COMMAND} -i "s/magi_app/${APP_CONTEXT_NAME}/g" $DESTINATION_PATH/../scripts/block_processing_healthcheck.sh
    fi
fi
