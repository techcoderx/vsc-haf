...
if [ "$1" = "install_app" ]; then
...
elif [ "$1" = "uninstall_subindexer" ]; then
  shift
  exec su - haf_admin -c "/app/scripts/uninstall_app.sh --subindexer --host=\"${POSTGRES_HOST:-haf}\" --user=\"${POSTGRES_USER:-haf_admin}\""
  ...
  echo "usage: $0 install_app|process_blocks|subindexer|uninstall_subindexer|uninstall_app"
  exit 1
fi