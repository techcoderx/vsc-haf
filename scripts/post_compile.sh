...
API_SCHEMA_NAME=vsc_api
SCHEMA_NAME=vsc_app
APP_CONTEXT_NAME=vsc_app
...
if [[ "$APP_CONTEXT_NAME" != "$DEFAULT_APP_CONTEXT_NAME" ]]; then
    ...
    ${SED_COMMAND} -i "s/${DEFAULT_APP_CONTEXT_NAME}/${APP_CONTEXT_NAME}/g" $DESTINATION_PATH/constants.js
fi
...
if [[ "$SCHEMA_NAME" != "$DEFAULT_SCHEMA_NAME" ]]; then
    ...
    ${SED_COMMAND} -i "s/${DEFAULT_SCHEMA_NAME};/${SCHEMA_NAME};/g" $DESTINATION_PATH/sql/*.sql
    ${SED_COMMAND} -i "s/${DEFAULT_SCHEMA_NAME} /${SCHEMA_NAME} /g" $DESTINATION_PATH/sql/*.sql
    ${SED_COMMAND} -i "s/${DEFAULT_SCHEMA_NAME}/${SCHEMA_NAME}/g" $DESTINATION_PATH/constants.js
fi
