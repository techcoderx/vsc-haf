...
CHECK="SET TIME ZONE 'UTC'; \
       SELECT ((now() - (SELECT last_active_at FROM hafd.contexts WHERE name = 'vsc_app')) < interval '1 minute' \
               AND (SELECT last_active_at FROM hafd.contexts WHERE name = 'vsc_app') > '${STARTUP_TIME}'::timestamp) OR \
              ((SELECT current_block_num FROM hafd.contexts WHERE name = 'vsc_app') >= (SELECT consistent_block FROM hafd.irreversible_data))"
