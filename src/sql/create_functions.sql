...
CREATE OR REPLACE FUNCTION magi_app.enum_op(IN _first_block INT, IN _last_block INT)
RETURNS SETOF magi_app.op_type
AS
$function$
BEGIN
...
DROP TYPE IF EXISTS magi_app.block_type CASCADE;
CREATE TYPE magi_app.block_type AS (
    num INTEGER,
    created_at TIMESTAMP
);

CREATE OR REPLACE FUNCTION magi_app.enum_block(IN _first_block INT, IN _last_block INT)
RETURNS SETOF magi_app.block_type
AS
$function$
BEGIN
    -- Fetch block headers
...
CREATE OR REPLACE FUNCTION magi_app.get_tx_hash_by_op(_block_num INTEGER, _trx_in_block SMALLINT)
RETURNS TEXT
AS
$function$
BEGIN
    RETURN (
        ...
        FROM magi_app.transactions_view t
...
CREATE OR REPLACE FUNCTION magi_app.process_operation(_username VARCHAR, _op_id BIGINT, _op_type INTEGER, _ts TIMESTAMP)
RETURNS BIGINT
AS
$function$
DECLARE
    ...
    _op_pos INTEGER;
BEGIN
    SELECT id INTO _hive_user_id FROM hafd.magi_app_accounts WHERE name=_username;
    IF _hive_user_id IS NULL THEN
        RAISE EXCEPTION 'Could not process non-existent user %', _username;
    END IF;

    SELECT count INTO _nonce FROM magi_app.l1_users WHERE id=_hive_user_id;
    IF _nonce IS NOT NULL THEN
        UPDATE magi_app.l1_users SET
            count=count+1,
            last_op_ts=_ts
        WHERE id=_hive_user_id;
    ELSE
        INSERT INTO magi_app.l1_users(id, count, last_op_ts)
            VALUES(_hive_user_id, 1, _ts);
    END IF;

    SELECT o.block_num, o.trx_in_block, o.op_pos
        INTO _block_num, _trx_in_block, _op_pos
        FROM magi_app.operations_view o
        WHERE o.id = _op_id;

    INSERT INTO magi_app.l1_operations(user_id, nonce, op_id, block_num, trx_in_block, op_pos, op_type, ts)
...
CREATE OR REPLACE FUNCTION magi_app.update_witness(
    _username VARCHAR,
...
BEGIN
    ...
    IF (SELECT EXISTS (SELECT 1 FROM magi_app.witnesses WHERE id = _hive_user_id)) THEN
        UPDATE magi_app.witnesses SET
        ...
        WHERE id = _hive_user_id;
    ELSE
        INSERT INTO magi_app.witnesses (id, consensus_did, peer_id, peer_addrs, version_id, git_commit, protocol_version, gateway_key, enabled, last_update, first_seen)
...
-- Helper function for parsing L1 payload mainly for use by magi_api schema
CREATE OR REPLACE FUNCTION magi_app.parse_l1_payload(_op_name VARCHAR, _op_body jsonb)
RETURNS jsonb AS $$
DECLARE
    _payload TEXT;
    _payload2 jsonb;
