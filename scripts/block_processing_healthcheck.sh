...
# - VSC-HAF has processed a block in the last 60 seconds
#   (as long as it was also after the container started, we don't want
#    to report healthy immediately after a restart)
# or
# - VSC-HAF's head block has caught up to haf's irreversible block
#   (so we don't mark VSC-HAF as unhealthy if HAF stops getting blocks)
#
...
CHECK="SET TIME ZONE 'UTC'; \
       SELECT ((now() - (SELECT last_active_at FROM hive.contexts WHERE name = 'vsc_app')) < interval '1 minute' \
               AND (SELECT last_active_at FROM hive.contexts WHERE name = 'vsc_app') > '${STARTUP_TIME}'::timestamp) OR \
              ((SELECT current_block_num FROM hive.contexts WHERE name = 'vsc_app') >= (SELECT consistent_block FROM hive.irreversible_data))"
